...
`src/` is the app — a React/TypeScript frontend, and the only code in scope for review. `mock-server/` and the mock-API plugin in `vite.config.ts` aren't part of the app; they're just faking a backend.
